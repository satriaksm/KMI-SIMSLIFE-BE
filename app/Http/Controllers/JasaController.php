...
class JasaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $jasas->each(function ($jasa) {
            ...
            if ($jasa->images && $jasa->images->count() > 0) {
                $isPublic = in_array($jasa->status, ['published', 'archived'], true);
    ...
    public function publicIndex(Request $request)
    {
        ...
        if (array_key_exists('min_price', $data) || array_key_exists('max_price', $data)) {
            ...
            $priceExpr = "COALESCE(NULLIF(fixed_price, 0), NULLIF(base_price, 0), 0)";
        ...
        switch ($sort) {
            case 'price_asc':
                $query->orderByRaw("COALESCE(NULLIF(fixed_price, 0), NULLIF(base_price, 0), 0) asc")
                    ->orderByDesc('id');
                break;
            case 'price_desc':
                $query->orderByRaw("COALESCE(NULLIF(fixed_price, 0), NULLIF(base_price, 0), 0) desc")
                    ->orderByDesc('id');
                break;
            case 'name_asc':
    ...
    public function update(Request $request, $id)
    {
        ...
        // Also normalize delivery_type if sent (convert old values to new standard)
        $st = $request->input('delivery_type') ?? $request->input('service_type');
        if ($st === 'at_location' || $st === 'ditempat_saya' || $st === 'di_tempat_umkm' || $st === 'in-store') {
            $request->merge(['delivery_type' => 'in-store']);
        } elseif ($st === 'on_site' || $st === 'kerumah_pelanggan' || $st === 'ke_rumah_pelanggan' || $st === 'on-site') {
            $request->merge(['delivery_type' => 'on-site']);
        } elseif ($st === 'online') {
            $request->merge(['delivery_type' => 'online']);
        }
        ...
        $validated = $request->validate([
            ...
            'delivery_type' => 'nullable|string|in:online,in-store,on-site',
        ...
        // delivery_type sudah dinormalisasi ke format baru di atas
        if (array_key_exists('delivery_type', $data)) {
            if ($data['delivery_type'] === 'in-store') {
            ...
            // on-site: location_address tetap nullable, customer isi saat booking
            if ($data['delivery_type'] === 'on-site') {
                $data['location_address'] = '';
            }
        }
        ...
        if (array_key_exists('status', $data)) {
            // Keep status as is (published/archived)
        }
        ...
        if (!empty($files)) {
            ...
            foreach ($files as $file) {
                $suffix = 'gallery-' . $order;
                $path = app(\App\Services\ImageOptimizationService::class)->processAndStore(
                    $file,
                    "jasas/{$jasa->slug}",
                    'public',
                    true,
    ...
    public function storeForMerchant(Request $request, int $merchantId)
    {
        ...
        // Normalize delivery_type (convert old values to new standard)
        $st = $request->input('delivery_type') ?? $request->input('service_type');
        if ($st === 'at_location' || $st === 'ditempat_saya' || $st === 'di_tempat_umkm' || $st === 'in-store') {
            $request->merge(['delivery_type' => 'in-store']);
        } elseif ($st === 'on_site' || $st === 'kerumah_pelanggan' || $st === 'ke_rumah_pelanggan' || $st === 'on-site') {
        ...
        $validated = $request->validate([
            ...
            // Tipe & lokasi layanan
            'delivery_type' => 'required|string|in:online,in-store,on-site',
            'location_address' => 'nullable|string|max:500',
            'special_notes' => 'nullable|string',
        ...
        // Sinkronkan alamat layanan sesuai tipe layanan
        if (($data['delivery_type'] ?? null) === 'in-store') {
            $primary = $merchant->primary_address;
            $location = trim(implode(', ', array_filter([
                data_get($primary, 'detail'),
                data_get($primary, 'village'),
        ...
        if (($data['delivery_type'] ?? null) === 'online' || ($data['delivery_type'] ?? null) === 'on-site') {
            $data['location_address'] = '';
        }
        ...
        if (!empty($files)) {
            if (!empty($files)) {
                ...
                foreach ($files as $index => $file) {
                    $suffix = ($index === 0) ? 'cover' : 'gallery-' . $index;
                    $path = app(\App\Services\ImageOptimizationService::class)->processAndStore(
                        $file,
                        "jasas/{$jasa->slug}",
                        'public',
